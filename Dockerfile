...
# then log output of file /code/bin/new1
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/new1 \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/new1 | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/new1 /usr/bin/new1
