...
HTTPS_GIT := github.com/abhi/abhishek.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=abhishek \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=new1 \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/new1 ./cmd/new1
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/new1.exe ./cmd/new1
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/new1
...
draw-deps:
	...
	@goviz -i ./cmd/new1 -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c abhishek --local -f chains.yaml
endif
...
set-testnet-configs:
	new1 config set client chain-id chainid-1
	new1 config set client keyring-backend test
	new1 config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | new1 keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | new1 keys add acc1 --recover`
